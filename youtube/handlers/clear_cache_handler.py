    ...
    def get(self):
        ...
        with logger.tags('ClearCache'):
            ...
            if clear_cache_requests:
                logger.info(f'Force clear cache started ({self.request.remote_ip})')
                needClear = True
            ...
            for cache_type, value in clear_cache_requests.items():
                if value == ClearCacheHandler.ALL:
                    ...
                    logger.info(f'Cleared all items from {cache_type} cache ({deleted_count} items deleted)')
                else:
                    deleted_count = youtube.youtube.cache_manager.cleanup_cache(cache_type, value)
                    logger.info(f'Cleared item \"{value}\" from {cache_type} cache ({deleted_count} items deleted)')

            if needClear:
                selfurl = f'{self.request.protocol}://{self.request.host}{self.request.path}'
                logger.info(f'Redirect to {selfurl}')
