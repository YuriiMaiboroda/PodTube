...
class LogFileHandler(StaticFileNonCacheHandler):
    ...
    def add_block_borders(string: str) -> str:
        string = re.sub(r"^\s*(\d+-\d+-\d+ \d+:\d+:\d+).*", rf"{LogFileHandler.BLOCK_START}\g<0>", string, flags=re.MULTILINE)
        string = re.sub(rf"^{LogFileHandler.BLOCK_START}(.|\n)*?(?=\n{LogFileHandler.BLOCK_START}|\Z)", rf"\g<0>{LogFileHandler.BLOCK_END}", string, flags=re.MULTILINE)
        return string
    ...
    def add_log_level_style(string: str):
        ...
        return re.sub(rf"^{LogFileHandler.BLOCK_START}.* \[(CRITICAL|ERROR|WARNING|INFO|DEBUG)\](.|\n)*?(?=\n{LogFileHandler.BLOCK_START}|\Z)", replacer, string, flags=re.MULTILINE)
